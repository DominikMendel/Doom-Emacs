...
;; Adding "--no-ignore" to allow to search ".gitignore" files
(after! projectile
  (setq projectile-git-fd-args "-tl -H -0 -E .git -tf --strip-cwd-prefix -c never --no-ignore"))
...
;; Add zoxide entries after visting them
(add-hook 'find-file-hook #'zoxide-add) ;; I don't think this adds any directory information
(add-hook 'projectile-after-switch-project-hook #'zoxide-add) ;; This does work

;; Camel case jumping
(setq global-subword-mode t)

;; Stop pasting for overwriting the register
(setq evil-kill-on-visual-paste nil)

;; BUG I don't think this is doing anything
(after! treemacs
...
** Custom surrounds
#+begin_src elisp
(defun my/get-matching-delimiters (left-delim right-delim)
  ;; Allows going from "<" + ">" to 'test<Surrounded>'
  (let ((base-delimiter (evil-surround-read-from-minibuffer
                         "Enter the left delimiter to be surrounded by: (Can add multiple, i.e., 'Array Gd') "
                         "")))
    (cons (format "%s%s" (or base-delimiter "") left-delim)
          right-delim)))

;; Not tested, don't know how to delete/find/change
(defun my/find-for-surrounding (left-delim right-delim)
  "Find text matching the pattern defined by LEFT-DELIM and RIGHT-DELIM."
  (let ((pattern (format "[^=%s(){}]+%b" left-delim right-delim)))
    (list pattern)))

;; Not tested, don't know how to delete/find/change
(defun my/delete-for-surrounding (left-delim right-delim)
  "Construct the delete pattern for LEFT-DELIM and RIGHT-DELIM."
  (format "^(.*?\\%s)(.*?)(\\%s)$" left-delim right-delim))



(defun my/get-angled-surround ()
  (my/get-matching-delimiters "<" ">"))
(defun my/get-paren-surround ()
  (my/get-matching-delimiters "(" ")"))
(defun my/get-bracket-surround ()
  (my/get-matching-delimiters "[" "]"))
(defun my/get-generic-surround ()
  (let ((left-delimiter (evil-surround-read-from-minibuffer "Enter the left delimiter: "))
        (right-delimiter (evil-surround-read-from-minibuffer "Enter the right delimiter: ")))
    (cons left-delimiter right-delimiter)))

(after! evil-embrace
  (push '(?< . my/get-angled-surround) evil-surround-pairs-alist)
  (push '(?( . my/get-paren-surround) evil-surround-pairs-alist)
  (push '(?[ . my/get-bracket-surround) evil-surround-pairs-alist)
  (push '(?i . my/get-generic-surround) evil-surround-pairs-alist)

  ;; BUG This one doesn't work - Probably different mode hook
  ...
  ;; (embrace-add-pair ?E "#+begin_example\n" "#+end_example\n")
  ;; (add-hook! 'org-mode-hook :append
  ;; ;; (add-hook! 'embrace-org-mode-hook :append
  ;;   (embrace-add-pair ?E "#+begin_example\n" "#+end_example\n"))
  ;; )
#+end_src
...
;;   ;; (setq +corfu-want-tab-prefer-expand-snippets t) ;; Doesn't do anything
;;   )
#+end_src
* Deft
#+begin_src elisp
...
(custom-set-faces!
  ...
  ;; '(vterm-color-blue :foreground "color-51") ;; For doom-gruvbox
...
** General org
#+begin_src elisp
;; This fixes org clock functions
(add-hook! 'org-mode-hook
...
(use-package! org-roam-dailies
  ...
  :config
        ...
        (defun my/mark-done-at-scheduled-time ()
            ...
            (define-advice org-add-log-setup (:after (&rest _) trick-store-note)
            (setq org-log-note-effective-time thetime))
            ;; TODO This doesn't work, but should? It sets the date wrong
            ;; (when (eq major-mode 'org-agenda-mode) ;if we are in agenda mode we switch to orgmode
            ;;   (org-agenda-show)
            ;;   (org-agenda-goto))
            (org-todo 'done)
  ...
  (defun my/org-clock-in-at-specified-time (&rest _)
    ...
    (let* ((thetime (org-read-date))
      ...
      (when (org-clock-is-active)
        ...
        (save-window-excursion
                (cond
                 ((eq major-mode 'org-agenda-mode) ;if we are in agenda mode we switch to
                       ;; Have to manually go to the org-entry from org-agenda, and recheck the value
                       ;; Since org-agenda doesn't get if tags from the let* scope
                    (org-agenda-show)
                    (org-agenda-goto)
                    (setq org-id-link-to-org-use-id (if (-contains? tags "journal") "use-existing" t))
                    (org-clock-in nil timestamp)
                    (org-roam-dailies--capture timestamp nil "!CI"))
                ((eq major-mode 'org-mode) ; do this only in org-mode
                        (org-clock-in nil timestamp)
                        (org-roam-dailies--capture timestamp nil "!CI"))))))
  ...
  (defun my/org-clock-out-at-specified-time (&optional time &rest _)
    ...
    (let* ((thetime (or time (org-read-date)))
      ...
      ;; Manually jump to the current clock, for the %a in the template
        (save-window-excursion
                (save-excursion
                (org-clock-goto)
...
(defun my/kill-all-other-buffer (&optional _)
  (interactive)
...
(defun my/org-roam-sync (&optional _)
  (interactive)
  (let* ((org-ids-path (concat org-directory "/" ".orgids")))
         (message "Deleting org-ids at : %s" org-ids-path)
         ;; May want to add "let" (restore-buffer-modified-p nil) to remove modified buffers
         ;; (restore-buffer-modified-p nil)
         (delete-file org-ids-path)
...
;; (defcustom my/org-capture-bookmark nil "SomeDocument")
(defun my/test-save-variable ()
  (interactive)
  (customize-save-variable 'my/org-capture-bookmark t "SomeDocument"))

(defvar my/org-capture-bookmark nil "SomeDocument")
;; Allows my custom variable to persist accross sessions
(after! savehist
  (add-to-list
   'savehist-additional-variables
   'my/org-capture-bookmark))

(defvar my/git-project-note-file-path nil
  "A string system path to perform org-captures related to a git project.
A roam note should already be created.")
(defvar my/git-project-note-capture-name "Default"
  "A string that is used in org-capture as the default ertny location.
This will be updated to the relative file path")

(defvar org-journal--date-location-scheduled-time nil)

;; TODO Complete
(defun my/git-project-capture ()
  (interactive)
  (let ((original-kill-ring (copy-sequence kill-ring))
        (+default/yank-buffer-path-relative-to-project)
        (let ((my/git-project-note-capture-name (copy-sequence kill-ring))
              )
          (org-capture nil "pp")
          )
    ...
    (cond ((string-equal dm--my-focus "vispero") (setq +org-capture-todo-file dm--org-vispero-inbox-file-name))
      ...
      (setq org-capture-templates
        `(("l" "Japanese")
          ...
          ("m" "Meeting ToDo" entry (file+headline dm--org-sirius-meetings-file-name "Non-repeating meetings")
           "* TODO %c%?\nSCHEDULED: %^{Schedule}t")
          ...
          ("p" "Project templates")
          ;; TODO Alter this
          ("pp" "Git Project" entry (file+headline my/git-project-note-file-path my/git-project-note-capture-name)
           ;; "* TODO %^{What ToDo?}%? :new:\n%i\nFrom : %a\n")
           "* %U %?\n%i\n%a"
           :prepend t)
...
;; Add relative line numbers to modes that don't usually have line numbers
(add-hook 'org-agenda-mode-hook #'my/enable-relative-line-numbers)
(add-hook 'dired-mode-hook #'my/enable-relative-line-numbers)
(add-hook 'magit-mode-hook #'my/enable-relative-line-numbers)
...
;; I used to use this before I needed to configure the "_" to spaces.
;; Vulpea functions to help with naming view
;; (setq org-agenda-prefix-format
;;       '((agenda . " %i %(vulpea-agenda-category 1)%?-20t% s")
;;         (todo . " %i %(vulpea-agenda-category 1) ")
;;         (tags . " %i %(vulpea-agenda-category 1) ")
;;         (search . " %i %(vulpea-agenda-category 1) ")))

;; Customize: max length for filename display
(defvar my/org-agenda-fname-max 20
  "Max characters to show for file names in the agenda.")

(defun my/org-agenda-filename-prefix ()
  "Return a short filename string for org-agenda prefix.
Replaces underscores with spaces, capitalizes the first letter of each word,
and truncates to `my/org-agenda-fname-max`. If truncated, appends an ellipsis."
  (let* ((file (or (buffer-file-name (current-buffer))
                   (or (buffer-name) "")))
         ;; Get base name without directory and extension
         (base (file-name-base file))
         ;; Replace underscores with spaces
         (repl (replace-regexp-in-string "_" " " base))
         ;; Capitalize the first letter of each word
         (capitalized (mapconcat 'capitalize (split-string repl) " "))
         ;; Trim leading/trailing spaces
         (trim (string-trim capitalized))
         ;; Truncate if too long
         (len (length trim)))
    (if (<= len my/org-agenda-fname-max)
        trim
      (concat (substring trim 0 (- my/org-agenda-fname-max 1)) "…"))))


(setq org-agenda-prefix-format
      ;; '((agenda . " %i %-12(my/org-agenda-filename-prefix):") ;; QUESTION not sure if the extra stuff is needed?
      '((agenda . " %i %-12(my/org-agenda-filename-prefix): %?-20t% s")
        (timeline . " %i %-20(my/org-agenda-filename-prefix): %s")
        (todo . " %i %-12(my/org-agenda-filename-prefix): ")
        (tags . " %i %-12(my/org-agenda-filename-prefix): ")
        (search . " %i %-12(my/org-agenda-filename-prefix): ")))
...
Refer to `org-agenda-prefix-format' for more information."
  (let* ((file-name (when buffer-file-name
         ...
         (category (if (derived-mode-p 'org-mode) (org-get-category) nil)) ;; For org bug
         (result
          (or (if (and
                   title
...
(defun my-error-logging-hook (orig-fun &rest args)
  "A function that logs errors and the current buffer."
  (condition-case err
      (apply orig-fun args)
    (error
     (message "Error in %s\nError: %s"
...
;; Hook the target function with the error-logging hook
;; (advice-add 'org-get-category :around 'my-error-logging-hook)
...
(setq my-super-group-agenda-today
      '(
        ...
        (:order-multi (5
                       ...
                       (:name "Previously Scheduled: Waiting"
                        :and (:scheduled past :todo "WAITING"))
                       ))
        (:auto-parent t
         :order 6)
        (:discard (:anything t))))
...
* Dirvish - Dired 2.0
#+begin_src elisp
;; TODO Want to add dirvish-peek-mode by default - can see files if file viewer like Spc-p-f

;; Jump to quick access with "b" inside dirvish
...
;; Not used anymore, but good reference for filter function use
...
(defun my/org-roam-dailies-startup ()
  (interactive)
  (org-roam-dailies-capture-today nil "!JS")
  (my/org-clock-in-at-specified-time))

(defun my/org-roam-dailies-discussion ()
  (interactive)
  (org-roam-dailies-capture-today nil "!Jd")
        (end-of-visible-line)
        (evil-append 1)
        (insert " ")
        ;; Or can use : (vulpea-insert (lambda (note) (seq-contains-p (vulpea-note-tags note) "people")))
        (vulpea-insert (lambda (note) (vulpea-note-tagged-all-p note "people")))
        (evil-force-normal-state))

(defun my/org-roam-dailies-helping-person ()
  (interactive)
  (org-roam-dailies-capture-today nil "!Jh")
        (end-of-visible-line)
        (evil-append 1)
        ;; This is a bit of a hack to force proper spacing
        (insert " ")
        (org-set-tags ":helping:")
        ;; Or can use : (vulpea-insert (lambda (note) (seq-contains-p (vulpea-note-tags note) "people")))
        (insert " ")
        (vulpea-insert (lambda (note) (vulpea-note-tagged-all-p note "people")))
        (evil-force-normal-state))
...
(after! org-roam
    ...
    (defvar dm--org-sirius-meetings-file-name (expand-file-name "sirius_meetings.org" dm--org-roam-sirius-dir))
      ...
      ;; Set the dailies directory based on focus
        ...
        (setq org-roam-dailies-capture-templates
            ...
            `(("d" "default" plain "%?"
                ...
                ;; :clock-in t)
                :jump-to-captured t)
                ("!Jd" "Journal Discussion" entry "* Discussion with %?"
                :target (file+head+olp ,file-name ,head ("Journal"))
                :unnarrowed t
                :immediate-finish t
                ;; :clock-in t)
                :jump-to-captured t)
                ("!Jh" "Journal Helping Person" entry "* Helping %?"
                :target (file+head+olp ,file-name ,head ("Journal"))
                :unnarrowed t
                :immediate-finish t
                ;; :clock-in t)
...
(defun my/url-format (path1 path2 title &rest remaining)
  (concat path1 "/" path2 "/" (string-replace "/" "-" title) (if remaining (string-replace "/" "-" remaining)) ".org"))
...
(defun my/jama-test (url)
  ...
  (setq org-roam-capture-ref-templates
        ...
        ("j" "jira" plain "- tags :: [[roam:Sirius Jira]] %?\n* TODO ${title}\nSCHEDULED: %^{Schedule}t"
           ...
           ;; :if-new (file+head ,(my/url-format "sirius" "jira" "${title}") ":PROPERTIES:
           :if-new (file+head ,(my/url-format "sirius" "jira" "${slug}") ":PROPERTIES:
:ROAM_REFS: ${ref}
:END:
,#+TITLE: ${title}\n#+filetags: :jira:\n")
           :unnarrowed t)
...
** General
#+begin_src elisp
(define-key! :keymaps +default-minibuffer-maps [C-return] #'find-file-other-window)
...
(map! :leader
      ;; cmd! is Doom's shorthand of a interactive lambda
      "SPC" (cmd! (let ((current-prefix-arg t)) (evil-avy-goto-char-timer))) ;; This is also mapped to g-s-SPC
      ...
      "fy" #'+default/yank-buffer-path-relative-to-project
      "ps" #'+default/search-project
      :desc "Open project in terminal" "pP" (cmd!
                                       (let* ((+workspaces-switch-project-function #'+vterm/here))
                                         (projectile-switch-project)))
      :desc "Open project with magit" "pg" (cmd!
      ...
      :desc "Open directory in zoxide" "p z" #'zoxide-travel
      :desc "Find file in zoxide" "p Z" #'zoxide-find-file
      ;; TODO look into : +default/search-other-project
      ;; TODO make a function to pass highlight phrase based on visual select
      "bh" #'highlight-phrase
      "bH" #'unhighlight-regexp
...
;; Remove C-j movements for window movement
(map! :map 'c++-mode-map
...
(map! :map 'cpp-mode-map
      ...
      "C-l" #'evil-window-right
      "C-h" #'evil-window-left)
;; BUG C-j/k don't work for org agenda
(map!
 :after (:and org org-agenda evil)
 :map (org-agenda-mode-map org-agenda-keymap)
      "C-j" #'evil-window-down
      "C-k" #'evil-window-up
      "C-l" #'evil-window-right
      "C-h" #'evil-window-left
...
;; Set global window montion keys to a map with high priority
(map!
        :nvig "C-h" #'evil-window-left
        :nvig "C-j" #'evil-window-down
        :nvig "C-k" #'evil-window-up
        :nvig "C-l" #'evil-window-right)

(map!
        :nvig "M-h" #'evil-window-left
        :nvig "M-j" #'evil-window-down
        :nvig "M-k" #'evil-window-up
        :nvig "M-l" #'evil-window-right)

(map!
        :nvig "M-1" #'+workspace/switch-to-0
        :nvig "M-2" #'+workspace/switch-to-1
        :nvig "M-3" #'+workspace/switch-to-2
        :nvig "M-4" #'+workspace/switch-to-3
        :nvig "M-5" #'+workspace/switch-to-4
        :nvig "M-6" #'+workspace/switch-to-5
        :nvig "M-7" #'+workspace/switch-to-6
        :nvig "M-8" #'+workspace/switch-to-7
        :nvig "M-9" #'+workspace/switch-to-8
        :nvig "<f1>" #'+workspace/switch-to-0
        :nvig "<f2>" #'+workspace/switch-to-1
        :nvig "<f3>" #'+workspace/switch-to-2
        :nvig "<f4>" #'+workspace/switch-to-3
        :nvig "<f5>" #'+workspace/switch-to-4
        :nvig "<f6>" #'+workspace/switch-to-5
        :nvig "<f7>" #'+workspace/switch-to-6
        :nvig "<f8>" #'+workspace/switch-to-7
        :nvig "<f9>" #'+workspace/switch-to-8)
...
(defun my/clock-in-history ()
  (interactive)
  ;; 4 is C-u, 16 is C-u C-u, 64 is C-u C-u C-
  (org-clock-in '(4)))
...
(map!
 ...
 :map (org-mode-map org-agenda-mode-map)
        ...
        ;; :nvigm "M-k" nil
        ;; :nvigm "M-l" nil
        ;; :nvigm "M-S-h" 'org-metaleft
        ;; :nvigm "M-S-j" 'org-metadown
        ;; :nvigm "M-S-k" 'org-metaup
        ;; :nvigm "M-S-l" 'org-metaright
        ;; :nvigm "M-S-C-h" 'org-shiftmetaleft
        ;; :nvigm "M-S-C-j" 'org-shiftmetadown
        ;; :nvigm "M-S-C-k" 'org-shiftmetaup
        ;; :nvigm "M-S-C-l" 'org-shiftmetaright
 :localleader
 (:prefix ("c" . "clock")
  :desc "org-clock-in-at-time" "I" #'my/org-clock-in-at-specified-time
  :desc "org-clock-out-at-time" "O" #'my/org-clock-out-at-specified-time
  :desc "org-clock-in-history" "h" #'my/clock-in-history)
 :leader
...
;; BUG why doesn't this work?
      ;; "j" (cmd! (org-next-visible-heading) (recenter))
      ;; "J" (cmd! (my/org-next-parent-heading) (recenter))
      ;; "k" (cmd! (org-previous-visible-heading) (recenter))
      ;; "K" (cmd! (outline-up-heading) (recenter))
      ;; "RET" (cmd! (org-insert-subheading) (recenter)))
...
"
  ...
  ("m" (lambda()
         (interactive)
         (call-interactively (org-capture nil "m")))
   "Meeting ToDo")
)
#+end_src
*** Journal capture
...
"
  ...
  ("l" (lambda()
   ...
   "Lunch")
  ("s" my/org-roam-dailies-startup "Startup")
  ("d" my/org-roam-dailies-discussion "Discussion")
  ("h" my/org-roam-dailies-helping-person "Helping Person")
)
...
;;       "C-e" #'corfu-popupinfo-scroll-down
;;       "C-y" #'corfu-popupinfo-scroll-up)

;; Corfu tab work around?
;; (map!
;;  :i [tab]
...
** General
#+begin_src elisp
;; Add sticky hooks to see function definitions at top of line
...
(after! lsp-rust
  (setq
    lsp-rust-analyzer-display-type-hints t
    lsp-rust-analyzer-display-parameter-hints t
    lsp-rust-analyzer-display-chaining-hints t
    lsp-rust-analyzer-display-closure-parameter-hints t
...
(after! lsp-mode
  (setq lsp-inlay-hint-enable t)      ;; required for any hints to render
  (add-hook 'rustic-mode-hook #'lsp-inlay-hints-mode))  ;; turn them on in buffers
#+end_src
** C++
#+begin_src elisp
...
(defun my/markdown-live-view ()
  ;; Launches a live markdown preview in browser
  (interactive)
  (unless (httpd-running-p)
    (httpd-start))
  (impatient-mode 1)
  (imp-set-user-filter #'markdown-html)
  (browse-url "localhost:8080/imp"))
#+end_src
* Vterm and fish
#+begin_src elisp
;; (after! vterm
...
* SSH and Tramp
#+begin_src elisp
;; (after! tramp ((setq tramp-shell-prompt-pattern "\\(?:^\\|\r\\)[^]#$%>\n]*#?[]#$%>].* *\\(^[\\[[0-9;]*[a-zA-Z] *\\)*")))
#+end_src
* Garbage collection
#+begin_src elisp
...
;; BUG Alpha-background somehow not working for Arch
...
;; Add default search to projectile projects
(projectile-discover-projects-in-directory "~/repos/" 1)
(projectile-discover-projects-in-directory "~/projects/" 3)
(projectile-discover-projects-in-directory "~/Dropbox/coding/" 3)
...
;; Add default search to projectile projects
(projectile-discover-projects-in-directory "~/repos/" 2)
...
(setq doom-font (font-spec :family "Fira Code" :size 28)
      doom-variable-pitch-font (font-spec :family "Fira Code" :size 28)
      doom-unicode-font (font-spec :family "JuliaMono Medium" :size 28)
      ;; doom-unicode-font (font-spec :family "Fira Code" :size 28)
